package com.xkmxz.tongdarailway_for_forge.structure;
    ...
    private static class NBTResourceReloadListener extends SimplePreparableReloadListener<Map<ResourceLocation, CompoundTag>> {
        ...
        private void init() {
            LOADED_STRUCTURES.forEach((location, compoundTag) -> {
                Tongdarailway_for_forge.LOGGER.info("Loading station template: {}", location.getPath());
                    ...
                    switch (subString[1].split("/")[0]) {
                        case "normal":
                            ...
                            if (normalStation.getExitCount() == 4) {
                                ...
                                Tongdarailway_for_forge.LOGGER.warn("Invalid station structure. Exits count must 4: {} (exit count: {})", location.getPath(), normalStation.getExitCount());
                            }
                            break;
                        case "underground":
                            ...
                            if (undergroundStation.getExitCount() == 4) {
                                ...
                                Tongdarailway_for_forge.LOGGER.warn("Invalid station structure. Exits count must 4: {} (exit count: {})", location.getPath(), undergroundStation.getExitCount());
                            }
                            break;
                    }
                }
